...
### Verbessert
- **Platzhalter-Erklärung im Hilfetext-Editor übersichtlicher.** Die Liste
  zeigt Platzhalter und Bedeutung jetzt in einer klaren Tabelle
  (`{{kontakt}}` → die hinterlegte Kontakt-E-Mail usw.), statt in einem
  Fließtext, der zum versehentlichen Mitkopieren einlud. So ist eindeutig, dass
  z. B. nur `{{kontakt}}` in den Text gehört – nicht der Erklärungszusatz
  dahinter.
...
### Verbessert
- **Logo lädt viel schneller.** Statt bei jedem Seitenaufruf neu geladen zu
  werden, wird das Logo jetzt lange im Browser zwischengespeichert (1 Tag,
  „immutable"), mit ETag/Last-Modified für 304-Antworten. Die Logo-URL trägt
  eine Versionskennung (Änderungszeit der Datei) – sie wechselt nur, wenn
  wirklich ein neues Logo hochgeladen wurde, sonst nutzt der Browser den Cache.
  (Das frühere `?Zeitstempel` erzwang bei jedem Aufruf einen Neuladen.)
- **„Meine Termine" lädt automatisch** – kein „Termine laden"-Knopf mehr,
  konsistent zu den übrigen Ansichten.
- **Neuladen bleibt auf der aktuellen Ansicht.** Die gewählte Ansicht steht
  jetzt im URL-Hash (z. B. `#meine`); ein Reload (F5) landet nicht mehr auf der
  Startseite. Vor/Zurück im Browser funktioniert ebenfalls. Beim Abmelden wird
  der Hash geleert.

### Hinweis
- Das hinterlegte Logo ist mit 256 KB relativ groß. Für Kopf/Anzeige genügt ein
  kleineres Bild (~15–30 KB); ein Verkleinern beschleunigt den Erstaufruf
  zusätzlich. (Optional, nicht erforderlich.)
